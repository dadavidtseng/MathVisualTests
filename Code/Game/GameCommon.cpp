...
void DebugDrawLine(Vec2 const& start, Vec2 const& end, float thickness, Rgba8 const& color)
{
    ...
    Vec3 vertA = Vec3(start.x - halfThicknessOffset.x, start.y - halfThicknessOffset.y, 0.f);
    Vec3 vertB = Vec3(start.x + halfThicknessOffset.x, start.y + halfThicknessOffset.y, 0.f);
    Vec3 vertC = Vec3(end.x + halfThicknessOffset.x, end.y + halfThicknessOffset.y, 0.f);
    Vec3 vertD = Vec3(end.x - halfThicknessOffset.x, end.y - halfThicknessOffset.y, 0.f);

    Vertex_PCU verts[6];

    verts[0].m_position = vertA;
    verts[1].m_position = vertB;
    verts[2].m_position = vertC;
    verts[0].m_color    = color;
    verts[1].m_color    = color;
    verts[2].m_color    = color;

    verts[3].m_position = vertA;
    verts[4].m_position = vertC;
    verts[5].m_position = vertD;
