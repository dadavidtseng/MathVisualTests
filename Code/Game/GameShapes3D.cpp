...
GameShapes3D::GameShapes3D()
{
    ...
    m_testShapes.m_position    = Vec3(-2.f, 0.f, 1.f);
    m_testShapes.m_orientation = EulerAngles::ZERO;
    m_testShapes2.m_position    = Vec3(-2.f, 0.f, 1.f);
    m_testShapes2.m_orientation = EulerAngles::ZERO;
...
void GameShapes3D::Render() const
{
    ...
    AddVertsForSphere3D(verts, Vec3::ZERO, 0.1f);
    Mat44 m2w;

    m2w.SetTranslation3D(m_testShapes2.m_position);
    m2w.AppendZRotation(m_testShapes2.m_orientation.m_yawDegrees);
    m2w.AppendYRotation(m_testShapes2.m_orientation.m_pitchDegrees);
    m2w.AppendXRotation(m_testShapes2.m_orientation.m_rollDegrees);
...
void GameShapes3D::UpdateFromKeyboard(float deltaSeconds)
{
    ...
    Vec3 left;
    Vec3 up;
    m_testShapes.m_orientation.GetAsVectors_IFwd_JLeft_KUp(forward, left, up);

    m_testShapes.m_velocity                = Vec3::ZERO;
    ...
    if (g_theInput->IsKeyDown(KEYCODE_W)) m_testShapes.m_velocity += forward * moveSpeed;
    if (g_theInput->IsKeyDown(KEYCODE_S)) m_testShapes.m_velocity -= forward * moveSpeed;
    if (g_theInput->IsKeyDown(KEYCODE_A)) m_testShapes.m_velocity += left * moveSpeed;
    if (g_theInput->IsKeyDown(KEYCODE_D)) m_testShapes.m_velocity -= left * moveSpeed;
    ...
    m_testShapes.m_position += m_testShapes.m_velocity * deltaSeconds;
    ...
    m_testShapes.m_angularVelocity.m_rollDegrees = 0.f;
    ...
    if (g_theInput->IsKeyDown(KEYCODE_Q)) m_testShapes.m_angularVelocity.m_rollDegrees = 90.f;
    if (g_theInput->IsKeyDown(KEYCODE_E)) m_testShapes.m_angularVelocity.m_rollDegrees = -90.f;

    m_testShapes.m_orientation.m_rollDegrees += m_testShapes.m_angularVelocity.m_rollDegrees * deltaSeconds;
    m_testShapes.m_orientation.m_rollDegrees = GetClamped(m_testShapes.m_orientation.m_rollDegrees, -45.f, 45.f);

    m_worldCamera->SetPositionAndOrientation(m_testShapes.m_position, m_testShapes.m_orientation);
}
